...
Blockly.Arduino.procedures_defreturn = function() {
    ...
    //ADEL paramString this.argumentstype_[0];
    ...
    //ADEL
    //var testadel = this.argumentstype_[x];
    if (!returnValue) {
        dropdown_type = 'void';
    }
    var mesarguments = [];
    for (var x = 0; x < this.arguments_.length; x++) {
        mesarguments[x] = this.argumentstype_[x] + ' ' + this.arguments_[x];
    }


    var code = dropdown_type + ' ' + funcName + '(' + mesarguments.join(', ') + ') {\n' +
        branch + returnValue + '}\n'; //HERE
...
Blockly.Arduino.procedures_callnoreturn = function() {
    ...
    for (var x = 0; x < this.arguments_.length; x++) {
        args[x] = Blockly.Arduino.valueToCode(this, 'ARG' + x,
            Blockly.Arduino.ORDER_NONE) || 'null'; //ADEL
    }
    var code = funcName + '(' + args.join(', ') + ');\n';
    return code;
};
